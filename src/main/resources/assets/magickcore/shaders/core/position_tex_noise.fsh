...
void main() {
    ...
    vec4 noiseTex = texture(Sampler1, noiseUV);

    float time = GameTime*50.0;
    float c1 = noise_turbulence(vec3(texCoord0*2, time))*rgb2hsv(calibration(noiseTex.rgb)).z;
    vec4 noise = vec4( 1.5*c1, 1.5*c1*c1*c1, c1*c1*c1*c1*c1*c1, GameTime );
    float alpha = rgb2hsv(calibration(noise.rgb)).z;
    fragColor = vec4(color.rgb * ColorModulator.rgb, color.a * ColorModulator.a * alpha);
}
