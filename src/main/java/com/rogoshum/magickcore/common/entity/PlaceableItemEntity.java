...
public class PlaceableItemEntity extends Entity implements IEntityAdditionalSpawnData, IManaRefraction {
    ...
    public void onSyncedDataUpdated(DataParameter<?> key) {
        if (HEIGHT.equals(key) || WIDTH.equals(key) || DIRECTION.equals(key)) {
            ...
            if(this.getDirection().getAxis().isVertical() && this.getDirection().getAxisDirection().getStep() == -1) {
                ...
                pos = pos.add(Vector3d.atLowerCornerOf(getDirection().getNormal()).scale(d0)).subtract(0, getBbHeight() * 0.5, 0);
            }
            this.setBoundingBox(new AxisAlignedBB(pos.x - d0, pos.y, pos.z - d0, pos.x + d0, pos.y + (double)getBbHeight(), pos.z + d0));
        }
    ...
    public void nextNode(ItemStack stack, Direction direction) {
        ...
        Vector3d pos = position().add(Vector3d.atLowerCornerOf(getDirection().getOpposite().getNormal()).scale(-d1));
        if(direction.getAxis().isVertical()) {
            pos = pos.subtract(Vector3d.atLowerCornerOf(direction.getOpposite().getNormal()).scale(-d1));
        } else {
            pos = pos.subtract(Vector3d.atLowerCornerOf(direction.getOpposite().getNormal()).scale(-d0));
        }
    ...
    protected void addAdditionalSaveData(CompoundNBT compound) {
        ...
        compound.put("Item", getItemStack().save(new CompoundNBT()));
