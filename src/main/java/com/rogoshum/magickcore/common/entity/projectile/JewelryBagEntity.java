...
public class JewelryBagEntity extends ManaProjectileEntity implements IManaRefraction {
        ...
        if(spellContext().containChild(LibContext.TRACE) && !level.isClientSide()) {
            ...
            if (raytraceresult.getType() == HitResult.Type.BLOCK) {
                ...
                if(ModBlocks.ITEM_EXTRACTOR.isPresent() && block == ModBlocks.ITEM_EXTRACTOR.get()) {
                    this.setPos(blockHitResult.getBlockPos().getX() + 0.5, blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ() + 0.5);
                    this.remove();
                }
            } else {
    ...
    @Override
    public boolean hitBlockRemove(BlockHitResult blockHitResult) {
        ...
        if(tile instanceof Container) {
            if(spellContext().containChild(LibContext.ITEM)) {
                ...
                return super.hitBlockRemove(blockHitResult);
    ...
    @Override
    public boolean hitEntityRemove(EntityHitResult entityHitResult) {
        if(spellContext().containChild(LibContext.ITEM) && entityHitResult.getEntity() == getOwner())
            return true;
        if(entityHitResult.getEntity() instanceof ItemEntity) {
            if(spellContext().containChild(LibContext.ITEM)) {
                ItemEntity entity = (ItemEntity) entityHitResult.getEntity();
            ...
            return false;
        } else if(entityHitResult.getEntity() instanceof JewelryBagEntity) {
            return false;
        } else if (suitableEntity(entityHitResult.getEntity()) && (getOwner() == null || (getOwner() != null && getOwner() != entityHitResult.getEntity()))) {
            return false;
        }
        return super.hitEntityRemove(entityHitResult);
    }
