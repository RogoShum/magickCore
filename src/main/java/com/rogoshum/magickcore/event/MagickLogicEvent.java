...
public class MagickLogicEvent {
	...
	{
		if(event.getMagickType() == "EYE_STAR")
		{
			if(event.getEntity() instanceof PlayerEntity && !((PlayerEntity)event.getEntity()).getActivePotionMap().containsKey(ModEffects.TRACE.orElse(null)))
	...
	public void onWeakenBuff(LivingHurtEvent event)
	{
		...
		IEntityState attacker = event.getEntity().getCapability(MagickCore.entityState).orElse(null);
