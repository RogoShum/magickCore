    ...
    public void tick() {
        ...
        if(this.ticksExisted % 100 == 0) {
            ...
            for(LivingEntity living : entityList)
            {
                ...
                String magickType = getTraceTarget() == null ? "EYE_STAR" : "EYE_STAR_TRACE";
                ...
                starEntity.shoot(motion.x, motion.y, motion.z, 0.03f, 1.0f);
                ...
                if(this.getManaType().equals(EnumManaType.BUFF) && living instanceof PlayerEntity) {
                    starEntity.setTraceTarget(event.getTrace());
                    if (!this.world.isRemote())
                        this.world.addEntity(starEntity);
                }
                if((this.getManaType().equals(EnumManaType.DEBUFF) || this.getManaType().equals(EnumManaType.ATTACK)) &&!(living instanceof PlayerEntity)) {
                    starEntity.setTraceTarget(event.getTrace());
                    if (!this.world.isRemote())
                        this.world.addEntity(starEntity);
                }
