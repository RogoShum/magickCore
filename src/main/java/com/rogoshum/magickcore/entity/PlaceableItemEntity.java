...
public class PlaceableItemEntity extends Entity implements IEntityAdditionalSpawnData {
    ...
    public void remove() {
        ...
        if(this.origin == this) {
            this.entityMap.values().forEach(PlaceableItemEntity::remove);
        }
    ...
    public void nextNode(ItemStack stack, Direction direction) {
        ...
        if(this.getDirection().getAxis().isVertical() && this.getDirection().getAxisDirection().getOffset() == -1) {
            ...
            pos = pos.subtract(Vector3d.copy(getDirection().getOpposite().getDirectionVec()).scale(d0));
        }
