    ...
    public void tick() {
        ...
        if(this.spellContext().containChild(LibContext.DIRECTION)) {
            ...
            for(int i = 0; i < list.size(); ++i) {
                ...
                if(!refraction) {
                    MagickContext attribute = new MagickContext(this.world)
                            .noCost()
                            .caster(this.getOwner()).projectile(this).victim(entity)
                            .tick(spellContext().tick)
                            .force(spellContext().force)
                            .range(spellContext().range)
                            .applyType(ApplyType.HIT_ENTITY)
