...
public class ChaoReachEntity extends ManaPointEntity implements ISuperEntity {
    ...
    public void releaseMagick() {
        ...
        for (Entity entity : livings) {
            makeSound = true;
            MagickContext context = new MagickContext(world).noCost().caster(this.getOwner()).projectile(this).victim(entity).tick(50).force(EnumManaLimit.FORCE.getValue()).applyType(ApplyType.DE_BUFF);
            MagickReleaseHelper.releaseMagick(context);
            context = new MagickContext(world).noCost().caster(this.getOwner()).projectile(this).victim(entity).tick(10).force(EnumManaLimit.FORCE.getValue()).applyType(ApplyType.ATTACK);
            MagickReleaseHelper.releaseMagick(context);
        }
