    ...
    public void releaseMagick() {
        ...
        for (Entity living : list)
        {
            ...
            if(living.isAlive() && !state.getOwnerUUID().equals(this.getOwnerUUID()) && !MagickReleaseHelper.sameLikeOwner(this.getOwner(), living)) {
                MagickContext context = new MagickContext(this.world).noCost().caster(this.getOwner()).projectile(this).victim(living).tick(this.spellContext().tick / 4).force(EnumManaLimit.FORCE.getValue()).applyType(ApplyType.HIT_ENTITY);
                MagickReleaseHelper.releaseMagick(context);
                context = new MagickContext(this.world).noCost().caster(this.getOwner()).projectile(this).victim(living).tick(this.spellContext().tick / 4).force(EnumManaLimit.FORCE.getValue()).applyType(ApplyType.ATTACK);
                MagickReleaseHelper.releaseMagick(context);
            }
        }
    }
