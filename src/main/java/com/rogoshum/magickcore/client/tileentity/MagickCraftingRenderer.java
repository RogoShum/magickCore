...
public class MagickCraftingRenderer implements BlockEntityRenderer<MagickCraftingTileEntity> {
    ...
    public void render(MagickCraftingTileEntity tileEntityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        RenderHelper.renderStaticParticle(BufferContext.create(matrixStackIn, Tesselator.getInstance().getBuilder(), type), new RenderHelper.RenderContext(0.15f * alpha, Color.BLUE_COLOR, RenderHelper.renderLight));
        matrixStackIn.scale(1.17f, 1.17f, 1f);
        type = RenderHelper.getTexedOrbGlow(ModElements.ORIGIN.getRenderer().getOrbTexture());
        RenderHelper.renderStaticParticle(BufferContext.create(matrixStackIn, Tesselator.getInstance().getBuilder(), type), new RenderHelper.RenderContext(0.1f * alpha, Color.BLUE_COLOR, RenderHelper.renderLight));
