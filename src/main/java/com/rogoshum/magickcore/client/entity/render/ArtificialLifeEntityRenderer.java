...
public class ArtificialLifeEntityRenderer extends EntityRenderer<ArtificialLifeEntity> {
	...
	public ArtificialLifeEntityRenderer(EntityRendererProvider.Context renderManager) {
		super(renderManager);
	}
	...
	public void render(ArtificialLifeEntity entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
		...
		Minecraft.getInstance().getItemRenderer().renderStatic(stack, ItemTransforms.TransformType.GROUND, RenderHelper.renderLight, OverlayTexture.NO_OVERLAY, matrixStackIn, bufferIn, 0);
		matrixStackIn.popPose();
		matrixStackIn.pushPose();
		matrixStackIn.translate(0, -entityIn.getBbHeight() * 0.5, 0);
		matrixStackIn.scale(4, 4, 4);
		Minecraft.getInstance().getItemRenderer().renderStatic(ITEM, ItemTransforms.TransformType.GROUND, packedLightIn, OverlayTexture.NO_OVERLAY, matrixStackIn, bufferIn, 0);
		matrixStackIn.popPose();
		matrixStackIn.popPose();
