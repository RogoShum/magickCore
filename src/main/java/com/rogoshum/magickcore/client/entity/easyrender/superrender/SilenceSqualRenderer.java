...
public class SilenceSqualRenderer extends EasyRenderer<SilenceSquallEntity> {
    ...
    public void render(SilenceSquallEntity entityIn, MatrixStack matrixStackIn, IRenderTypeBuffer.Impl bufferIn, float partialTicks) {
        ...
        if(entityIn.getElement() != null && entityIn.getElement().getRenderer() != null) {
            ...
            RenderHelper.renderCylinder(RenderHelper.getTexedCylinderGlint(entityIn.getElement().getRenderer().getWindTexture(0)), matrixStackIn, bufferIn, 0.4f * alpha, entityIn.getElement().getRenderer().getColor()
                    , 8.0f, 2.35f, 6, true, entityIn.getUniqueID().toString(), 1f);

            RenderHelper.renderCylinder(RenderHelper.getTexedCylinderGlint(entityIn.getElement().getRenderer().getWindTexture(1)), matrixStackIn, bufferIn, 0.3f * alpha, entityIn.getElement().getRenderer().getColor()
                    , 3.85f, 4.25f, 5, true, entityIn.getUniqueID().toString(), 1f);

            RenderHelper.renderCylinder(RenderHelper.getTexedCylinderGlint(entityIn.getElement().getRenderer().getWindTexture(2)), matrixStackIn, bufferIn, 0.2f * alpha, entityIn.getElement().getRenderer().getColor()
                    , 6.0f, 6.35f, 4, true, entityIn.getUniqueID().toString(), 0f);

            RenderHelper.renderCylinder(RenderHelper.getTexedCylinderGlint(entityIn.getElement().getRenderer().getWindTexture(3)), matrixStackIn, bufferIn, 0.3f * alpha, entityIn.getElement().getRenderer().getColor()
