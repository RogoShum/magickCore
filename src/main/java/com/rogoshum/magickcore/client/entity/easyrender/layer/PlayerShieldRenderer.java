        ...
        if(state != null) {
            ...
            if (value > 0.0f) {
                ...
                state.getElement().getRenderer().renderSphere(
                        BufferPackage.create(matrixStackIn, Tessellator.getInstance().getBuffer(), type), 16, alpha, test, false, Integer.toString(entityIn.getEntityId()), packedLightIn);
                matrixStackIn.scale(0.56f, 0.56f, 0.56f);
                RenderHelper.renderParticle(
                        BufferPackage.create(matrixStackIn, Tessellator.getInstance().getBuffer(), RenderHelper.getTexedOrbGlow(new ResourceLocation(MagickCore.MOD_ID + ":textures/element/base/shield/element_shield_" + Integer.toString(entityIn.ticksExisted % 10) + ".png")))
