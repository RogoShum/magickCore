...
public class ManaEyeRenderer extends EasyRenderer<ManaEyeEntity>{
    ...
    @Override
    public void render(ManaEyeEntity entityIn, MatrixStack matrixStackIn, BufferBuilder bufferIn, float partialTicks) {
        ...
        if(entityIn.getElement() != null && entityIn.getElement().getRenderer() != null) {
            ...
            entityIn.getElement().getRenderer().renderSphere(BufferPackage.create(matrixStackIn, bufferIn, RenderHelper.getTexedSphereGlow(blank, 1f, 0f)), 4, 0.6f, test, 0.0f, packedLightIn);
        }
    }
}
