...
public class ChaosReachLaserRenderer extends EasyRenderer<ChaoReachEntity> {
    ...
    public void update() {
        ...
        if(entity.initial) {
            ...
            List<Entity> livings = entity.findEntity((living) -> living instanceof LivingEntity && !MagickReleaseHelper.sameLikeOwner(entity.getOwner(), living) && MagickReleaseHelper.canEntityTraceAnother(entity, living));
            for (Entity entity : livings) {
                Vector3d me = getEntityRenderVector(Minecraft.getInstance().getRenderPartialTicks()).add(0, this.entity.getHeight() * 0.5, 0);
    ...
    public void render(RenderParams params) {
        ...
        for (Vector3d vector3d : direction) {
            ...
            entity.spellContext().element.getRenderer().renderLaserParticle(
                    matrixStackIn, params.buffer, entity.spellContext().element.getRenderer().getElcTexture(1), 0.35f, (float) (vector3d.z * 2), 2.0f);
            matrixStackIn.pop();
        }
    }
