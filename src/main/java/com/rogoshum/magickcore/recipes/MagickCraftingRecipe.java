...
public class MagickCraftingRecipe {
    ...
    private final MultiBlockHelper.PredicatePattern<PlaceableItemEntity>[] pattern;
    private final SpawnResult spawnResult;

    public MagickCraftingRecipe(String[][][] recipe, MultiBlockHelper.PredicatePattern<PlaceableItemEntity>[] pattern, SpawnResult spawnResult) {
        this.recipe = recipe;
    ...
    public boolean match(Optional<PlaceableItemEntity>[][][] structure) {
        ...
        for (int y = 0; y < structure.length; ++y) {
            for (int i = 0; i < 4; ++i) {
                ...
                for (int r = 0; r < i; ++r) {
                    rotated = MultiBlockHelper.rotate(rotated);
                }

                if(MultiBlockHelper.correctStructure(recipe[y], pattern, rotated)) {
