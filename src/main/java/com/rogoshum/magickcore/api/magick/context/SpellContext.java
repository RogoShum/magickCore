...
public class SpellContext {
    protected MagickElement element = MagickRegistry.getElement(LibElements.ORIGIN);
    protected ApplyType applyType = ApplyType.NONE;
    protected SpellContext postContext = null;
    protected int tick;
    ...
    public <T extends SpellContext> T post(@Nullable SpellContext context) {
        if(context == null) {
    ...
    public <T extends SpellContext> T merge(SpellContext context) {
        this.tick(this.tick() + context.tick());
        this.range(this.range() + context.range());
        this.force(this.force() + context.force());
        if(context.applyType() != ApplyType.NONE)
            this.applyType(context.applyType());
        context.childContexts().values().forEach(this::addChild);
        return (T) this;
    }
    ...
    public void clear() {
        this.element(ModElements.ORIGIN);
    ...
    public void deserialize(CompoundTag tag, int depth) {
        ...
        tagHelper.ifContainInt("TICK", this::tick);
        tagHelper.ifContainFloat("RANGE", this::range);
        tagHelper.ifContainFloat("FORCE", this::force);
