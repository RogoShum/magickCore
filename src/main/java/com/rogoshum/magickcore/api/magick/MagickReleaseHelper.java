...
public class MagickReleaseHelper {
    ...
    public static boolean consumePlayerMana(LivingEntity player, float mana) {
        ...
        EntityEvents.MagickReleaseCheckEvent event = new EntityEvents.MagickReleaseCheckEvent(MagickContext.create(player.level).caster(player), mana);
    ...
    public static EntityEvents.MagickReleaseCheckEvent preReleaseMagickEvent(MagickContext context) {
        float manaNeed = Math.max(0, manaNeed(context) - context.reduceCost);
        EntityEvents.MagickReleaseCheckEvent event = new EntityEvents.MagickReleaseCheckEvent(context, context.noCost ? 0 : manaNeed);
        MinecraftForge.EVENT_BUS.post(event);
        return event;
    }

    public static EntityEvents.MagickPreReleaseEvent releaseMagickEvent(MagickContext context) {
        EntityEvents.MagickPreReleaseEvent event = new EntityEvents.MagickPreReleaseEvent(context);
        MinecraftForge.EVENT_BUS.post(event);
        return event;
    }
    ...
    public static boolean releaseMagick(MagickContext context, ManaFactor manaFactor) {
        ...
        EntityEvents.MagickReleaseCheckEvent preEvent = preReleaseMagickEvent(context);
        ...
        if(context.world != null) {
            Vec3 pos = null;
            if(context.caster != null)
                pos = context.caster.getEyePosition();
            if(context.projectile != null)
                pos = context.projectile.getEyePosition();
            if(context.containChild(PositionContext.TYPE)) {
                PositionContext positionContext = context.getChild(PositionContext.TYPE);
                pos = positionContext.pos;
            }
            if(pos != null) {
                List<Entity> list = context.world.getEntitiesOfClass(Entity.class, new AABB(pos, pos).inflate(32), entity -> entity instanceof IQuadrantEntity);
                for(Entity entity : list) {
                    IQuadrantEntity quadrant = (IQuadrantEntity) entity;
                    AABB aabb = new AABB(entity.getEyePosition(), entity.getEyePosition()).inflate(quadrant.range());
                    if(aabb.contains(pos)) {
                        quadrant.magnify(context);
                    }
                }
            }
        }
