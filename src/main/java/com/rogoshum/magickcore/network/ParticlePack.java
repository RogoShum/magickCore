...
public class ParticlePack extends EntityPack{
    ...
    private final EnumParticleType texture;
    ...
    public ParticlePack(PacketBuffer buffer) {
        ...
        texture = buffer.readEnumValue(EnumParticleType.class);
    ...
    public ParticlePack(int id, EnumParticleType texture, Vector3d position, float scaleWidth, float scaleHeight, float alpha, int maxAge, String element,
    ...
    public void doWork(Supplier<NetworkEvent.Context> ctx) {
        ...
        if(texture == EnumParticleType.MIST)
            res = renderer.getMistTexture();
        else if(texture == EnumParticleType.PARTICLE)
            res = renderer.getParticleTexture();
        else if(texture == EnumParticleType.ORB)
            res = renderer.getOrbTexture();
        else if(texture == EnumParticleType.TRAIL)
